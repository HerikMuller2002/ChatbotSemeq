...
def analysis(input_user):
    ...
    if subject == "anything_else":
        ...
        # interface
        with open(f"models\\modelos_intencoes\\modelo_solution\\modelo_device\\{subject}\\modelo_interface\\{device}\\intents.json",'r',encoding="UTF-8") as bd:
            list_device = json.load(bd)
        model_path = f"models\\modelos_intencoes\\modelo_solution\\modelo_device\\{subject}\\modelo_interface\\{device}\\model.h5"
        words_path = f"models\\modelos_intencoes\\modelo_solution\\modelo_device\\{subject}\\modelo_interface\\{device}\\words.pkl"
        classes_path = f"models\\modelos_intencoes\\modelo_solution\\modelo_device\\{subject}\\modelo_interface\\{device}\\classes.pkl"
        intent_device = class_prediction(input_user, model_path,words_path,classes_path)
