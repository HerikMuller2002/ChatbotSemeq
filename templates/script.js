    ...
    sendMessage(userMessage).then((res, e) => {
        ...
        text.forEach((letter, i) => {
            ...
            textArray.map((item, j) => {
                ...
                count += 1
                setTimeout(() => {
                    textBot[textBot.length - 1].innerHTML += item + '<br>'
            ...
            setTimeout(() => {
                if ('link' in letter) {
                    // link = letter.link
                    textBot[textBot.length - 1].innerHTML += letter.text //+ '<br>' + `<a href="${link}" target="_blank" class="pdf__link">Link para o pdf</a>.` + '<br>'+'<br>'
                } else {
                textBot[textBot.length - 1].innerHTML += letter.text + '<br>'+ '<br>'
                textBot[textBot.length - 1].scrollIntoView()
                }
                if (text.length - 1 == i) {
                    setTimeout(() => {
        ...
        link = letter.link
        textBot[textBot.length - 1].innerHTML += `<a href="${link}" target="_blank" class="pdf__link">Link para o pdf</a>.` + '<br>'
